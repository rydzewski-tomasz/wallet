...
describe('ExpenditureSubcategory unit tests', () => {
  it('GIVEN expenditureSubcategory WHEN update name THEN update category name', async () => {
    ...
    // THEN
    const expectedExpenditureSubcategory = expenditureSubcategoryBuilder(expenditureSubcategory.toSnapshot()).withName('new category name').valueOf();
    expectEntity(expenditureSubcategory).toHaveEqualValue(expectedExpenditureSubcategory);
  });
});
