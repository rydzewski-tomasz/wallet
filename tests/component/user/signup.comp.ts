...
import { expectEntity, expectResponse } from '../../common/util/expectUtil';
...
describe('signup component test', () => {
  ...
  it('GIVEN valid request WHEN signup THEN save user on db', async () => {
    ...
    const expected = userBuilder()
      .withUuid(uuid)
      .withStatus(UserStatus.New)
      .withLogin('test')
      .valueOf();
    expect({ ...onDb, isValidPassword: await bcrypt.compare('pass', onDb.passwordHash) })
      .toStrictEqual({ ...expected.toSnapshot(), passwordHash: expect.any(String), isValidPassword: true })
  });
});
